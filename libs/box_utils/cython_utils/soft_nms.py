...
def py_soft_nms(dets, method='linear', iou_thr=0.3, sigma=0.5, score_thr=0.001):
    ...
    dets_tmp = np.concatenate((dets, areas[:, np.newaxis]), axis=1).copy()
    order=np.argsort(dets_tmp[:,4])[::-1]
    # or_sc=np.concatenate(scores[:,np.newaxis],order[:,np.newaxis])
    keep=[]
    while order.size > 2:
        # order=np.argsort(dets_tmp[:,4])[::-1]
        # keep.append(order[0])
        # max_box=dets_tmp[0,:4]
        # keep.append(max_idx)
        ...
        order=order[1:]
        id_in_order=np.argsort(scores[order])[::-1]
        order=order[id_in_order]
