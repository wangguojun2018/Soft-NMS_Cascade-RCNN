...
class DetectionNetwork(object):
    ...
    def build_fastrcnn(self, feature_to_cropped, rois, img_shape, scope):
        ...
        with tf.variable_scope('Fast-RCNN_{}'.format(scope)):
            ...
            if self.base_network_name.startswith('resnet'):
                ...
                net = slim.fully_connected(fc_flatten, 2048, weights_initializer=cfgs.INITIALIZER,scope='fc_1_{}'.format(scope))
                # net = slim.dropout(net, keep_prob=0.5, is_training=self.is_training, scope='dropout_{}'.format(scope))
                fc_flatten= slim.fully_connected(net, 2048,weights_initializer=cfgs.INITIALIZER, scope='fc_2_{}'.format(scope))
            else:
    ...
    def build_loss(self, rpn_box_pred, rpn_bbox_targets, rpn_cls_score, rpn_labels,
        ...
        with tf.variable_scope('build_loss') as sc:
            ...
            loss_dict = {
                ...
                'bbox_loss_1':bbox_loss,
                'bbox_loss_2':stage1_bbox_loss,
                'bbox_loss_3':stage2_bbox_loss,
                'cls_loss_1':cls_loss,
                'cls_loss_2':stage1_cls_loss,
                'cls_loss_3':stage2_cls_loss
            }
        return loss_dict
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch):
        ...
        stage1_bbox = self.postprocess_cascade(rois=stage1_rois,
                                               ...
                                               scope='stage1')#stage1最终计算得到的bbox
        ...
        if self.is_training:
            with tf.control_dependencies([stage1_bbox]):
                with tf.variable_scope('sample_RCNN_minibatch_stage2'):
                    ...
                    # fg_size = tf.shape(tf.where(tf.greater(stage2_labels,0)))[0]
                    # bg_size = tf.shape(tf.where(tf.equal(stage2_labels,0)))[0]
                    # print("stage 2 fg rois is ", fg_size, " bg rois is ", bg_size)
        else:
        ...
        if self.is_training:
            with tf.control_dependencies([stage2_bbox]):
                with tf.variable_scope('sample_RCNN_minibatch_stage3'):
                    ...
                    # fg_size=tf.shape(tf.where(tf.greater(stage3_labels,0)))[0]
                    # bg_size=tf.shape(tf.where(tf.equal(stage3_labels,0)))[0]
                    # print("stage 3 fg rois is ",fg_size," bg rois is ",bg_size)
        else:
    ...
    def get_restorer(self):
        ...
        if checkpoint_path != None:
            ...
            checkpoint_path='/home/wangguojun/source_code/py/Cascade-RCNN_Tensorflow/data/pretrained_weights/mobilenet/mobilenet_v2_1.0_224.ckpt'
