...
V2_BASE_DEF = dict(
    ...
    spec=[
        ...
        op(ops.expanded_conv, stride=1, num_outputs=96, scope='expanded_conv_12') #224:14*14*1280
    ],
)
...
V2_HEAD_DEF = dict(
    ...
    spec=[
        ...
        op(slim.conv2d, stride=1, kernel_size=[1, 1], num_outputs=1280, scope='Conv_1') #224:7*7*1280 本文中是B×4*4*1280
    ],
)
    ...
    if stage == 'stage1':
        with slim.arg_scope(mobilenetv2_scope(is_training=is_training, trainable=True)):
            flatten, _ = mobilenet_v2.mobilenet(input_tensor=inputs,
                                                ...
                                                finegrain_classification_mode=False)#有global average pooling输出1*1*1280
