...
def resnet_base(img_batch, scope_name, is_training=True):
    ...
    with slim.arg_scope(resnet_arg_scope(is_training=False)):
        with tf.variable_scope(scope_name, scope_name):
            ...
            net = tf.pad(net, [[0, 0], [1, 1], [1, 1], [0, 0]],name='pad_pooling')
            net = slim.max_pool2d(
                net, [3, 3], stride=2, padding='VALID', scope='pool1')

    not_freezed = [False] * cfgs.FIXED_BLOCKS + (4-cfgs.FIXED_BLOCKS)*[True] #第一个block不进行更新
...
def restnet_head(input, is_training, scope_name, stage):
    if stage == 'stage1':
        ...
        with slim.arg_scope(resnet_arg_scope(is_training=is_training)):
            ...
            flatten = tf.reduce_mean(C5, axis=[1, 2], keep_dims=False, name='global_average_pooling')#(B,2048)
        ...
        net = slim.fully_connected(fc_flatten, 2048, scope='fc_1_{}'.format(stage))
        # net = slim.dropout(net, keep_prob=0.5, is_training=is_training, scope='dropout_{}'.format(stage))
        flatten = slim.fully_connected(net, 2048, scope='fc_2_{}'.format(stage))
    return flatten
